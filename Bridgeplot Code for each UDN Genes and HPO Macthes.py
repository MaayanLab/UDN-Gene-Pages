...
for key, value in UDN_dict.items():
    ...
    # extract HP ids
    df_sorted = dff
    df_sorted["HP_ID"] = [x[1].replace(")", "") for x in df_sorted["Property"].astype(str).str.split("(")]
    ...
    #get only first 
    generif_dff = generif_graph.head(250)
    ...
    #get only first 
    autorif_dff = autorif_graph.head(250)
    ...
    #make dataframe for graphs
    tagger_graph = pd.DataFrame({ 'Property': tagger_a})
    
    #get only first 
    tagger_dff = tagger_graph.head(250)
    ...
    #get only first 
    archs4_dff = archs4_graph.head(250)
